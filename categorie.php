...
if (isset($_POST['ajouter_categorie'])) {
    if (!empty($_POST['categories'])) {
        ...
        $recup_categorie = $pdo->prepare('SELECT * FROM categories WHERE id = ?');
        $recup_categorie->execute([$categories]);

        if ($recup_categorie->rowCount() == 0) {
            $insert_categorie = $pdo->prepare('INSERT INTO categories (nom_quiz) VALUES (?)');
...
if (isset($_POST['supprimer_categorie'])) {
    if (!empty($_POST['categories'])) {
        $categories = htmlspecialchars($_POST['categories']);
        $delete_categorie = $pdo->prepare('DELETE FROM categories WHERE id = ?');
...
if (isset($_POST['ajouter_quiz'])) {
    if (!empty($_POST['nom_quiz']) && !empty($_POST['description_quiz']) && !empty($_POST['categories'])) {
        ...
        if ($categorie_data) {
            ...
            $insert_quiz = $pdo->prepare("INSERT INTO categories (nom_quiz, description_quiz, id_categorie) VALUES (?, ?, ?)");
...
<header>
        ...
        <select name="categories" required>
            ...
            <?php foreach ($categories as $categories) { ?>
                <option value="<?= htmlspecialchars($categories['id']) ?>">
                    <?= htmlspecialchars($categories['nom_quiz']) ?>
                </option>
            <?php } ?>
        </select>
    ...
    <form method="post">
        ...
        <select name="categories" required>
            <option value="">Sélectionner une catégorie</option>
            <?php foreach ($categories as $categories) { ?>
                <option value="<?= htmlspecialchars($categories['nom_categorie']) ?>">
                    <?= htmlspecialchars($categories['id']) ?>
                </option>
            <?php } ?>
        </select>
